...
def compute_josephson_current(Vvecs , Uvecs , Evals , ts , beta):
    current = np.zeros(len(ts) , dtype='complex')
    for vvec , uvec , eval in zip(Vvecs.T, Uvecs.T , Evals.T):
...
def main():
    ...
    N = sys.argv[4]
    beta = 6 
    Junction = SNS_junction(LS=101, LN=int(N), V=V, t=1, U0S=-V/2.0 + np.float64(mus), U0N=np.float64(mun), InverseTemp=beta , edge_sites_with_fixed_phase=5)
    ...
    for dphi in dphis:
        ...
        current = compute_josephson_current(Vvecs.T , Uvecs.T , Evals.T , ts , beta)
