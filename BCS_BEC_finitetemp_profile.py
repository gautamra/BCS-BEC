...
def symmetric_SNS_junction(LS, LN, V, t, U0S, U0N, InverseTemp , delphi, edge_sites_with_fixed_phase=5):
    LL_list = [LS, LN, LS]
    t_list = [t, t, t]
    v_list = [V, 0, V]
    u0_list = [U0S, U0N, U0S]
    ...
    return TBmodel(LL, ts, u0s, vs, beta=InverseTemp, constrain_phase=constrain_phase, Delta_init=Delta_init)

class SNS_junction:
    def __init__(self, LS, LN, V, t , U0S, U0N , InverseTemp='inf', edge_sites_with_fixed_phase=5):
        self.LS=LS
        self.LN=LN
        self.V=V
        self.t=t
    ...
    def get_Model_full(self, delphi , InverseTemp):
        ...
        return TBmodel(self.LL, ts, u0s, vs, beta=InverseTemp , constrain_phase=lambda Delta: self.constrain_phase(Delta, delphi), Delta_init=Delta_init)

    def get_Model_piece(self, piece , InverseTemp):
        if piece=='S':
            ts, u0s, vs = homogeneous_chain(self.LS, self.t, self.V, self.U0S, True)
            return TBmodel(self.LS, ts, u0s, vs , beta=InverseTemp)
        elif piece=='N':
            ts, u0s, vs = homogeneous_chain(self.LN, self.t, 0, self.U0N, True)
            return TBmodel(self.LN, ts, u0s, vs , beta=InverseTemp)
        else:
            raise ValueError('piece can only by S or N in a symmetric SNS junction')
            return


def compute_josephson_current(Vvecs , Uvecs , Evals , Delta , ts , beta):
    current = np.zeros(len(ts) , dtype='complex')
    for vvec , uvec , eval in zip(Vvecs, Uvecs , Evals):
        current += -2.0 * (np.array(ts) * ((1 - Fermi(eval , beta)) * (np.roll(vvec, 1) - np.roll(vvec, -1)) * (vvec.conjugate()) + (Fermi(eval , beta)) * (np.roll(uvec, 1) - np.roll(uvec, -1)) * (uvec.conjugate()))).imag
        #current += 4.0 * (1 - 2*Fermi(eval , beta))*np.cumsum((np.array(Delta) * (uvec.conjugate() * vvec))).imag
    return current
...
def main():
    ...
    mun = sys.argv[2]
    LN = 10
    beta = np.float64(sys.argv[3])
    Junction = SNS_junction(LS=101, LN=int(LN), V=V, t=1, U0S=-V/2.0 + np.float64(mus), U0N=np.float64(mun), InverseTemp=beta , edge_sites_with_fixed_phase=5)

    ModelS = Junction.get_Model_piece('S' , beta)
    ModelN = Junction.get_Model_piece('N' , beta)
    ...
    ts = [1.0]*(2*101 + int(LN))
    for dphi in dphis:
        Model = Junction.get_Model_full(delphi=dphi , InverseTemp=beta)
        resS = ModelS.iterate()
        resN = ModelN.iterate()
        ...
        current = compute_josephson_current(Vvecs.T , Uvecs.T , Evals.T , Del , ts , beta)
        ...
        positions = np.arange(1, len(Del)+1)
        current_date = datetime.datetime.now().strftime('%Y%m%d')
        folder_name = f"Current_Delta_profile_{current_date}_mus={mus}_mun={mun}_L={LN}_beta={beta}"
        os.makedirs(folder_name, exist_ok=True)
        output_plot_path = os.path.join(folder_name, f'Current_mus={mus}_mun={mun}_L={LN}_beta={beta}.png')
        output_excel_path = os.path.join(folder_name, f'Current_mus={mus}_mun={mun}_L={LN}_beta={beta}.xlsx')
        df = pd.DataFrame({'positions': positions, 'currents': current , 'Delta': Del , 'Potential': Pot})
    ...
    os.makedirs(folder_name, exist_ok=True)
    output_plot_path = os.path.join(folder_name, f'Current_mus={mus}_mun={mun}_L={LN}.png')
    output_excel_path = os.path.join(folder_name, f'Current_mus={mus}_mun={mun}_L={LN}.xlsx')
